...
async def upload_doc(file: UploadFile = File(...)):
    ...
    final_state = run_codegen_workflow(saved_path, OUTPUT_DIR)
    project_root = final_state.get("project_dir")
    project_zip = final_state.get("zip_path")
    
    port = get_free_port()
    container_url = f"http://localhost:{port}"
    container_id = build_and_run_podman_container(project_root, port)
    ...
    return {
        "message": "SRS processed successfully",
        "download_url": f"/download/{project_root}",
        "live_url": container_url,
        "endpoints": endpoints,
    }
...
def download_zip(file_name: str):
    zip_file_path = os.path.join("output_zip", f"{file_name}.zip")
    if os.path.exists(zip_file_path):
        return FileResponse(zip_file_path, media_type="application/zip", filename=f"{file_name}.zip")
    return {"error": "File not found"}
